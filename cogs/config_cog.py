...
class ConfigCog(commands.Cog):
    ...
    @commands.command(name="allrole", aliases=["addallrole", "giveroletoall"])
    async def allrole(self, ctx, role: discord.Role):
        """Ajoute un rôle à tous les membres (hors bots). Admin Discord ou rôle admin configuré."""
        ...
        status = await ctx.send(embed=discord.Embed(
            title="⏳ Ajout rôle",
            description=f"Ajout de {role.mention} à tous les membres…",
        ...
        for i, m in enumerate(humans, start=1):
            if role in m.roles:
                ...
                continue
            try:
                await m.add_roles(role, reason=f"allrole par {ctx.author}")
                done += 1
            ...
            if total >= 30 and i in (10, 25, 50, 100, 200, 400, 800):
                try:
                    await status.edit(embed=discord.Embed(
                        title="⏳ Ajout rôle",
                        description=f"Progression: **{i}/{total}**\nAjoutés: **{done}** · Déjà: **{skipped}** · Échecs: **{failed}**",
                        color=await get_guild_color(ctx.guild.id),
                    ))
                except Exception:
                    pass
        await status.edit(embed=success_embed(
            "Rôles",
            f"✅ Terminé : ajoutés **{done}**, déjà **{skipped}**, échecs **{failed}**.",
